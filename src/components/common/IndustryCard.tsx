    ...
    <div className="bg-white border border-slate-200 overflow-hidden group hover:shadow-lg transition-all duration-300 flex flex-col justify-between border-l-4 border-l-[#0A192F] hover:border-l-[#F27D26]">
            ...
            <div className="space-y-2 pt-3 border-t border-slate-100">
              ...
              <ul className="space-y-1.5">
                {industry.solutions.slice(0, 2).map((sol, idx) => (
                  <li key={idx} className="text-xs text-slate-700 flex items-start gap-2 line-clamp-1">
      ...
      <div className="p-4 bg-slate-50 border-t border-slate-100">
        <Link
          ...
          className="text-xs font-bold uppercase tracking-wider text-[#0A192F] hover:text-[#F27D26] transition-colors inline-flex items-center gap-1.5 w-full justify-between"
        >
