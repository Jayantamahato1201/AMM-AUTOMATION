...
export const AdminLayout: React.FC = () => {
  ...
  const navItems = [
    ...
    { name: 'Portfolio & Projects', path: '/admin/projects', icon: Layers },
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row text-slate-900 font-sans">
      {/* Mobile Top Nav */}
      <div className="md:hidden bg-[#0A192F] text-white p-4 flex items-center justify-between border-b border-slate-800">
      ...
      {/* Sidebar Navigation */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-[#0A192F] text-slate-300 transform transition-transform duration-200 ease-in-out md:translate-x-0 md:static md:inset-auto flex flex-col justify-between border-r border-slate-800 ${
      ...
      >
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-md text-xs font-semibold transition-colors ${
                    isActive
                      ? 'bg-blue-900/60 text-white border-l-4 border-orange-500 font-bold'
        ...
        <div className="p-4 border-t border-slate-800 bg-[#071324] space-y-3">
          <div className="flex items-center justify-between text-xs">
            <div>
              <p className="font-semibold text-white truncate max-w-[140px]">{user.name}</p>
              <span className="text-[10px] text-emerald-400 uppercase font-mono">{user.role}</span>
            </div>
            <button
          ...
          <Link
            to="/"
            target="_blank"
            className="w-full bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-[11px] font-medium py-2 px-3 rounded flex items-center justify-center gap-1.5 transition-colors border border-slate-700"
          >
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto max-h-screen">
        ...
        <header className="hidden md:flex bg-white border-b border-slate-200 px-8 py-4 items-center justify-between shadow-xs">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">AMM Automation Administration</span>
